 ...
 */
template <typename T, typename E = std::string> class expected {
public:
   ...
   */
  constexpr expected(const T &value) : value_(std::in_place_index<0>, value) {}
  ...
  constexpr auto
  and_then(Func &&func) & -> decltype(func(std::declval<T &>())) {
    if (has_value()) {
      return func(value());
    }
  ...
  template <typename Func>
  constexpr auto
  and_then(Func &&func) const & -> decltype(func(std::declval<const T &>())) {
    if (has_value()) {
      return func(value());
    }
  ...
  template <typename Func>
  constexpr auto
  map(Func &&func) & -> expected<decltype(func(std::declval<T &>())), E> {
    if (has_value()) {
